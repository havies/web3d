...
Ext.onReady(function() {
	...
	var filterCMTypeCombo = new Ext.form.ComboBox({
		name : 'filterCMtype',
		hiddenName : 'filterCMtype',
		typeAhead : true,
		triggerAction : 'all',
		lazyRender : true,
		mode : 'local',
	...
	var grid_part = new Ext.grid.EditorGridPanel({
				title : '<span class="commoncss">UI元素列表</span>',
				height : 500,
				autoScroll : true,
				region : 'center',
				store : store_part,
	...
	menuTreePanel.on('click', function(node) {
				...
				store_part.load({
							params : {
								...
								menuid : menuid,
								roleid : roleid,
								cmptype: filterCMTypeCombo.getValue()
							}
						});
			});
