...
import com.boidea.framework.core.metatype.util.TypeCaseHelper;
...
public class BFUtils {
	...
	private static Log log = LogFactory.getLog(BFUtils.class);
	...
	private static PropertiesHelper pHelper = PropertiesFactory.getPropertiesHelper(PropertiesFile.BF);
	...
	public static boolean isTheStyle(String pStr, String pStyle) {
		for (int i = 0; i < pStr.length(); i++) {
			...
			if (pStyle.equals(BFConstants.S_STYLE_N)) {
				if (!Character.isDigit(c))
					return false;
			} else if (pStyle.equals(BFConstants.S_STYLE_L)) {
				if (!Character.isLetter(c))
					return false;
			} else if (pStyle.equals(BFConstants.S_STYLE_NL)) {
	...
	public static String getClientExplorerType(HttpServletRequest request) {
		...
		if (isIE(request)) {
			...
			int index = userAgent.indexOf("safari");
			explorer = userAgent.substring(index, index + 13);
		}
	...
	public static String getGBK(String pString){
		...
		try {
			...
			log.error(BFConstants.Exception_Head + "不支持的字符集编码");
			e.printStackTrace();
		}
		return pString;
	}
	 ...
	 * @return
	 */
	public static boolean validateSQLInjection(String sqlStr) {
		if(BFUtils.isNotEmpty(sqlStr)) {
			sqlStr = sqlStr.toLowerCase();
			if(sqlStr.indexOf("insert") != -1) return false;
			if(sqlStr.indexOf("delete") != -1) return false;
			if(sqlStr.indexOf("drop") != -1) return false;
			if(sqlStr.indexOf("truncate") != -1) return false;
			if(sqlStr.indexOf("update") != -1) return false;
			if(sqlStr.indexOf("create") != -1) return false;
			if(sqlStr.indexOf("alter") != -1) return false;
			if(sqlStr.indexOf("$") != -1) return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	public static Object[] getDistinctArr(Object[] objArr) {
