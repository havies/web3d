...
function setCookie(name, value, minuts) {
    ...
    var expiration = new Date((new Date()).getTime() + minuts * 10000 * 60);
	/*
...
//下载VFS文件
function downloadAttachment(ctx, fileName, filePath) {
	var downloadIframe = document.createElement('iframe');
    downloadIframe.src = ctx + '/common/vfsfile.do?reqCode=downloadFile&fname='
                            +encodeURI(encodeURI(fileName))+'&fpath=' 
 ...
 * 判断是什么文件格式
 */
function isFileType(filetype, isFiletype) {
	var filetypeArr = {
			"pic":['ico','3ds','bmp','drw','dxf','eps','gif','jpg','jpeg','psd','wmf','pcd','pcx','psp','png','rle','raw','sct','tga','tif','tiff','u3d','xbm'],
			"zip":['zip','tar','rar','gz','apk'],
			"txt":['txt','ini','bat','html','htm']
	}
	if(filetypeArr[isFiletype]) {
		for(var i = 0; i < filetypeArr[isFiletype].length; i++) {
			if(filetypeArr[isFiletype][i] == filetype.toLowerCase()) {
				return true;
			}
		}
	}
...
Ext.ux.comboBoxRenderer = function(combo, fontColor) {
  return function(value) {
    var idx = combo.store.find(combo.valueField, value);
    var rec = combo.store.getAt(idx);
    if(fontColor)
    	return (rec == null ? '' : '<font color="'+fontColor+'">'+rec.get(combo.displayField) )+'</font>';
    else 
...
//导出窗口
function exportExcelWindow(exportUrl, columnStore) {
	var exportItemList = new Array();
	var allColValues = new Array();
	var allColNames = new Array();
	for (var i = 0; i < columnStore.getCount(); i++) {
		var colValue = columnStore.getAt(i).get('value');
		var colText = columnStore.getAt(i).get('text');
		if(mainGridHeader[colValue] === true) {
			allColValues.push(colValue);
			allColNames.push(colText);
		}
	}
	var allColValuesObj = new Object();
	allColValuesObj.name = "allColValues";
	allColValuesObj.xtype = "textfield";
	allColValuesObj.value = allColValues.join();
	allColValuesObj.hidden = true;
	exportItemList.push(allColValuesObj);
	var allColNamesObj = new Object();
	allColNamesObj.name = "allColNames";
	allColNamesObj.xtype = "textfield";
	allColNamesObj.value = allColNames.join();
	allColNamesObj.hidden = true;
	exportItemList.push(allColNamesObj);
	for (var i = 0; i < columnStore.getCount(); i++) {
		var colValue = columnStore.getAt(i).get('value');
		var colText = columnStore.getAt(i).get('text');
		if(mainGridHeader[colValue] === true) {
			var exportItem = new Object();
			exportItem.name = colValue;
			exportItem.boxLabel = colText;
			exportItem.xtype = "checkbox";
			exportItem.checked = true;
			exportItemList.push(exportItem);
		}
	}
	...
	var exportWindow = new Ext.Window({
		...
		draggable : true,
		closeAction : 'hide',
		title : '<span class="commoncss">选择导出项目</span>',
		modal : true,
		maximizable : true,
		titleCollapse : false,
		buttonAlign : 'right',
		...
		buttons : [{
			text : '导出',
			id : 'id_export_button_confirm',
			iconCls : 'page_excelIcon',
			handler : function() {
				exportForm.form.submit({
					url : exportUrl+'&oper=subcolumn',
					waitTitle : '提示',
					method : 'POST',
