...
Ext.onReady(function() {
	...
	var expander = new Ext.grid.RowExpander({
		tpl : new Ext.Template(
			...
			mainGridHeader.projectid === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_projectid+'：</span></td><td><span>{projectid}</span></td></tr>':'',
			mainGridHeader.userid === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_userid+'：</span></td><td><span>{userid}</span></td></tr>':'',
			'</table>'
			),
	...
	if(mainGridHeader.projectid === true)
        curColConfig.push({
                    header : mainGridHeader.header_projectid,
                    dataIndex : 'projectid',
                    sortable: true,
                    hidden: mainGridHeader.hide_projectid,
                    width : 150
        });
	if(mainGridHeader.userid === true)
        curColConfig.push({
                    header : mainGridHeader.header_userid,
                    dataIndex : 'userid',
                    sortable: true,
                    hidden: mainGridHeader.hide_userid,
                    width : 150
        });
    curColConfig.push({
                dataIndex : '_blank',
		    ...
		    'beforerender': function (m) {
		          for (var i = 0; i < project2userColumnStore.getCount(); i++) {
		        	  var nItem = new Ext.menu.CheckItem({
		        		  text: project2userColumnStore.getAt(i).get('text'),
		        		  checked : false,
		        		  hideOnClick: false,
	...
	function selectSearchItem(checked, itemText) {
		var itemValue = '';
		for (var i = 0; i < project2userColumnStore.getCount(); i++) {
      	  if(itemText == project2userColumnStore.getAt(i).get('text')) {
      		  itemValue = project2userColumnStore.getAt(i).get('value');
      		  break;
      	  }
        }
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span class="commoncss">project2user列表</span>',
				height : 500,
				// width:600,
				autoScroll : true,
				region : 'center',
						...
						*/
						...
						new Ext.form.TextField({
							...
							handler : function() {
								...
								exportExcelWindow(exportUrl, project2userColumnStore);
							}
						}],
				bbar : bbar
			});
	...
	var addWindow = new Ext.Window({
				...
				draggable : true,
				closeAction : 'hide',
				title : '<span class="commoncss">新增project2user</span>',
				modal : true,
				collapsible : true,
				titleCollapse : false,
				maximizable : true,
	...
	function addInit() {
		...
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">新增project2user</span>');
	...
	function saveItem() {
		...
		addFormPanel.form.submit({
					...
					success : function(form, action) {
						...
						store.reload();
						form.reset();
						Ext.Msg.alert('提示', '新增project2user成功');
					},
	...
	function editInit() {
		...
		if (Ext.isEmpty(record)) {
			Ext.Msg.alert('提示', '请先选中要修改的project2user');
			return;
		}
		addFormPanel.getForm().loadRecord(record);
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">修改project2user</span>');
	...
	function updateItem() {
		...
		addFormPanel.form.submit({
                    ...
                    success : function(form, action) {
                        ...
                        store.reload();
                        form.reset();
                        Ext.Msg.alert('提示', '修改project2user成功');
                    },
                    failure : function(form, action) {
                        top.showException(action.response.responseText);
                    }
	...
	function deleteItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.alert('提示', '请先选中要删除的project2user!');
			return;
		}
		var strChecked = jsArray2JsString(rows, 'uuid');
		Ext.Msg.confirm('请确认', '确认删除选中的project2user吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							...
							success : function(response) {
								...
								store.reload();
								Ext.Msg.alert('提示', '删除project2user成功');
							},
							params : {
