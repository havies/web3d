...
Ext.onReady(function() {
	...
	var expander = new Ext.grid.RowExpander({
		tpl : new Ext.Template(
			...
			mainGridHeader.materialname === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_materialname+'：</span></td><td><span>{materialname}</span></td></tr>':'',
	...
	if(mainGridHeader.materialname === true)
        curColConfig.push({
                    header : mainGridHeader.header_materialname,
                    dataIndex : 'materialname',
                    sortable: true,
                    hidden: mainGridHeader.hide_materialname,
                    width : 150
        });
		    ...
		    'beforerender': function (m) {
		          for (var i = 0; i < materialsColumnStore.getCount(); i++) {
		        	  var nItem = new Ext.menu.CheckItem({
		        		  text: materialsColumnStore.getAt(i).get('text'),
		        		  checked : false,
		        		  hideOnClick: false,
	...
	function selectSearchItem(checked, itemText) {
		var itemValue = '';
		for (var i = 0; i < materialsColumnStore.getCount(); i++) {
      	  if(itemText == materialsColumnStore.getAt(i).get('text')) {
      		  itemValue = materialsColumnStore.getAt(i).get('value');
      		  break;
      	  }
        }
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span class="commoncss">materials列表</span>',
				height : 500,
				// width:600,
				autoScroll : true,
				region : 'center',
						...
						*/
						...
						new Ext.form.TextField({
							...
							handler : function() {
								...
								exportExcelWindow(exportUrl, materialsColumnStore);
							}
						}],
				bbar : bbar
			});
	...
	var addWindow = new Ext.Window({
				...
				draggable : true,
				closeAction : 'hide',
				title : '<span class="commoncss">新增materials</span>',
				modal : true,
				collapsible : true,
				titleCollapse : false,
				maximizable : true,
	...
	function addInit() {
		...
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">新增materials</span>');
	...
	function saveItem() {
		...
		addFormPanel.form.submit({
					...
					success : function(form, action) {
						...
						store.reload();
						form.reset();
						Ext.Msg.alert('提示', '新增materials成功');
					},
	...
	function editInit() {
		...
		if (Ext.isEmpty(record)) {
			Ext.Msg.alert('提示', '请先选中要修改的materials');
			return;
		}
		addFormPanel.getForm().loadRecord(record);
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">修改materials</span>');
	...
	function updateItem() {
		...
		addFormPanel.form.submit({
                    ...
                    success : function(form, action) {
                        ...
                        store.reload();
                        form.reset();
                        Ext.Msg.alert('提示', '修改materials成功');
                    },
                    failure : function(form, action) {
                        top.showException(action.response.responseText);
                    }
	...
	function deleteItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.alert('提示', '请先选中要删除的materials!');
			return;
		}
		var strChecked = jsArray2JsString(rows, 'materialid');
		Ext.Msg.confirm('请确认', '确认删除选中的materials吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							...
							success : function(response) {
								...
								store.reload();
								Ext.Msg.alert('提示', '删除materials成功');
							},
							params : {
