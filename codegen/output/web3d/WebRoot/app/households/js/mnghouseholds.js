...
Ext.onReady(function() {
	...
	var expander = new Ext.grid.RowExpander({
		tpl : new Ext.Template(
			...
			mainGridHeader.roomtype === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_roomtype+'：</span></td><td><span>{roomtype}</span></td></tr>':'',
			mainGridHeader.square === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_square+'：</span></td><td><span>{square}</span></td></tr>':'',
	...
	if(mainGridHeader.roomtype === true)
        curColConfig.push({
                    header : mainGridHeader.header_roomtype,
                    dataIndex : 'roomtype',
                    sortable: true,
                    hidden: mainGridHeader.hide_roomtype,
                    width : 150
        });
	if(mainGridHeader.square === true)
        curColConfig.push({
                    header : mainGridHeader.header_square,
                    dataIndex : 'square',
                    sortable: true,
                    hidden: mainGridHeader.hide_square,
                    width : 150
        });
		    ...
		    'beforerender': function (m) {
		          for (var i = 0; i < householdsColumnStore.getCount(); i++) {
		        	  var nItem = new Ext.menu.CheckItem({
		        		  text: householdsColumnStore.getAt(i).get('text'),
		        		  checked : false,
		        		  hideOnClick: false,
	...
	function selectSearchItem(checked, itemText) {
		var itemValue = '';
		for (var i = 0; i < householdsColumnStore.getCount(); i++) {
      	  if(itemText == householdsColumnStore.getAt(i).get('text')) {
      		  itemValue = householdsColumnStore.getAt(i).get('value');
      		  break;
      	  }
        }
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span class="commoncss">households列表</span>',
				height : 500,
				// width:600,
				autoScroll : true,
				region : 'center',
						...
						*/
						...
						new Ext.form.TextField({
							...
							handler : function() {
								...
								exportExcelWindow(exportUrl, householdsColumnStore);
							}
						}],
				bbar : bbar
			});
	...
	var addWindow = new Ext.Window({
				...
				draggable : true,
				closeAction : 'hide',
				title : '<span class="commoncss">新增households</span>',
				modal : true,
				collapsible : true,
				titleCollapse : false,
				maximizable : true,
	...
	function addInit() {
		...
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">新增households</span>');
	...
	function saveItem() {
		...
		addFormPanel.form.submit({
					...
					success : function(form, action) {
						...
						store.reload();
						form.reset();
						Ext.Msg.alert('提示', '新增households成功');
					},
	...
	function editInit() {
		...
		if (Ext.isEmpty(record)) {
			Ext.Msg.alert('提示', '请先选中要修改的households');
			return;
		}
		addFormPanel.getForm().loadRecord(record);
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">修改households</span>');
		Ext.getCmp('windowmode').setValue('edit');
		Ext.getCmp('houseid').setValue(record.get('houseid'));
	}
	...
	function updateItem() {
		...
		addFormPanel.form.submit({
                    ...
                    success : function(form, action) {
                        ...
                        store.reload();
                        form.reset();
                        Ext.Msg.alert('提示', '修改households成功');
                    },
                    failure : function(form, action) {
                        top.showException(action.response.responseText);
                    }
	...
	function deleteItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.alert('提示', '请先选中要删除的households!');
			return;
		}
		var strChecked = jsArray2JsString(rows, 'houseid');
		Ext.Msg.confirm('请确认', '确认删除选中的households吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							...
							success : function(response) {
								...
								store.reload();
								Ext.Msg.alert('提示', '删除households成功');
							},
							params : {
