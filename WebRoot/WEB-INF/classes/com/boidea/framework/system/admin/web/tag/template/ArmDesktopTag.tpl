...
<link rel="stylesheet" type="text/css"
	href="${contextPath}/resource/css/framework.css" />
<link rel="stylesheet" type="text/css"
...
<script type="text/javascript" src="${contextPath}/resource/extjs/ext-all.js"></script>
  #end
<script type="text/javascript" src="${contextPath}/resource/commonjs/framework.js"></script>
...
#end
#window_CALENDAR001-shortcut img {
    width: 32px;
    height: 32px;
    background-image: url(${contextPath}/resource/desktop/images/shortcut/calendar.png);
    filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='${contextPath}/resource/desktop/images/shortcut/calendar.png',
        sizingMethod='scale' );
}
html, body {
...
<body scroll="no">
	...
	<div id="x-desktop">
		...
		#end
		    <dt id="window_CALENDAR001-shortcut">
                <a href="#"><img src="${contextPath}/resource/image/ext/s.gif" />
                    <div class="commoncss">日历</div></a>
            </dt>
		</dl>
    </div>
...
<script type="text/javascript">
	MyDesktop = new Ext.app.App({
		...
		getModules : function() {
			...
			new MyDesktop.Window_CALENDAR001(),  
			#foreach($menu in $menuList)
		...
		getStartConfig : function() {
			return {
				title : '<span class=commoncss>${username}<!--[${account}](${rolespecName})--></span>',
				iconCls : 'userIcon',
				toolItems : [ {
					text : '皮肤设置',
					iconCls : 'bugIcon',
...
MyDesktop.Window_CALENDAR001 = Ext.extend(
                    Ext.app.Module,
                    {
                        id : 'window_CALENDAR001',
                        init : function() {
                            this.launcher = {
                                text : '日历',
                                iconCls : 'bogus',
                        ...
                        createWindow : function(src) {
                            var desktop = this.app.getDesktop();
                            var win = desktop.getWindow('window_CALENDAR001');
                            if (!win) {
                                win = desktop
                                        .createWindow({
                                            id : 'window_CALENDAR001',
                                            ...
                                            height : 650,
                                            bodyStyle : 'background-color:#FFFFFF',
                                            pageY : document.body.clientHeight / 2 - 650 / 2,
                                            ...
                                            constrainHeader : true,
                                            constrain: true,
                                            html : '<iframe id="topMain" scrolling="auto" frameborder="0" style="width:100%;height:100%;" src="common/gycalendarevent/calendarMain.action"></iframe>'
                                        });
                            }
                            win.show();
                        }
...
function initSelectRolespecWin() {
	if(manyRolespec != 'true') {
		Ext.Msg.alert('提示', '用户没有多重身份，无需切换');
		return;
	}
		Ext.Ajax.request( {
