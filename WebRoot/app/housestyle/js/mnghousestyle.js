...
Ext.onReady(function() {
	...
	var expander = new Ext.grid.RowExpander({
		tpl : new Ext.Template(
			...
			mainGridHeader.stylename === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_stylename+'：</span></td><td><span>{stylename}</span></td></tr>':'',
			'</table>'
			),
	...
	if(mainGridHeader.stylename === true)
        curColConfig.push({
                    header : mainGridHeader.header_stylename,
                    dataIndex : 'stylename',
                    sortable: true,
                    hidden: mainGridHeader.hide_stylename,
                    width : 150
        });
    curColConfig.push({
                dataIndex : '_blank',
		    ...
		    'beforerender': function (m) {
		          for (var i = 0; i < housestyleColumnStore.getCount(); i++) {
		        	  var nItem = new Ext.menu.CheckItem({
		        		  text: housestyleColumnStore.getAt(i).get('text'),
		        		  checked : false,
		        		  hideOnClick: false,
	...
	function selectSearchItem(checked, itemText) {
		var itemValue = '';
		for (var i = 0; i < housestyleColumnStore.getCount(); i++) {
      	  if(itemText == housestyleColumnStore.getAt(i).get('text')) {
      		  itemValue = housestyleColumnStore.getAt(i).get('value');
      		  break;
      	  }
        }
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span class="commoncss">housestyle列表</span>',
				height : 500,
				// width:600,
				autoScroll : true,
				region : 'center',
						...
						*/
						...
						new Ext.form.TextField({
							...
							handler : function() {
								...
								exportExcelWindow(exportUrl, housestyleColumnStore);
							}
						}],
				bbar : bbar
			});
	...
	var addWindow = new Ext.Window({
				...
				draggable : true,
				closeAction : 'hide',
				title : '<span class="commoncss">新增housestyle</span>',
				modal : true,
				collapsible : true,
				titleCollapse : false,
				maximizable : true,
	...
	function addInit() {
		...
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">新增housestyle</span>');
	...
	function saveItem() {
		...
		addFormPanel.form.submit({
					...
					success : function(form, action) {
						...
						store.reload();
						form.reset();
						Ext.Msg.alert('提示', '新增housestyle成功');
					},
	...
	function editInit() {
		...
		if (Ext.isEmpty(record)) {
			Ext.Msg.alert('提示', '请先选中要修改的housestyle');
			return;
		}
		addFormPanel.getForm().loadRecord(record);
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">修改housestyle</span>');
		Ext.getCmp('windowmode').setValue('edit');
		Ext.getCmp('styleid').setValue(record.get('styleid'));
	}
	...
	function updateItem() {
		...
		addFormPanel.form.submit({
                    ...
                    success : function(form, action) {
                        ...
                        store.reload();
                        form.reset();
                        Ext.Msg.alert('提示', '修改housestyle成功');
                    },
                    failure : function(form, action) {
                        top.showException(action.response.responseText);
                    }
	...
	function deleteItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.alert('提示', '请先选中要删除的housestyle!');
			return;
		}
		var strChecked = jsArray2JsString(rows, 'styleid');
		Ext.Msg.confirm('请确认', '确认删除选中的housestyle吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							...
							success : function(response) {
								...
								store.reload();
								Ext.Msg.alert('提示', '删除housestyle成功');
							},
							params : {
