...
import com.boidea.framework.core.resource.AbstractResourceFilter;
import com.boidea.framework.core.resource.ResourceManager;
import com.boidea.framework.core.resource.util.StringUtils;
...
public class ResourceFilter extends AbstractResourceFilter {
	...
	protected ResourceManager createResourceManager(FilterConfig pFilterConfig) {
		...
		if (StringUtils.hasLength(config)) {
			logger.debug("BF.Resource配置文件是:" + config);
		}
		...
		if (configProperties.isEmpty()) {
			java.io.InputStream defaultIS = ResourceFilter.class.getResourceAsStream("BF.DefaultResource.properties");
			if (defaultIS != null) {
				try {
					configProperties.load(defaultIS);
				} catch (IOException ex) {
					final String msg = "装载系统资源:" + "BF.DefaultResource.properties" + "失败!";
					logger.error(msg, ex);
					throw new java.lang.RuntimeException(msg, ex);
				} finally {
					try {
