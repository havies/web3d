                    ...
                    items: [{
                        ...
                        listeners: {
                            ...
                            'eventadd': {
                                fn: function(cp, rec){
                                	Ext.Ajax.request({
            							url : ctx + '/common/gycalendarevent/saveItem.action',
            							success : function(response) {
            								var resultArray = Ext.util.JSON.decode(response.responseText);
            								rec.data.EventId = resultArray.msg;
            								App.showMsg('事件['+ rec.data.Title +']增加成功');
                            ...
                            'eventupdate': {
                                fn: function(cp, rec){
                                    Ext.Ajax.request({
            							url : ctx + '/common/gycalendarevent/updateItem.action',
            							success : function(response) {
            								var resultArray = Ext.util.JSON.decode(response.responseText);
            								App.showMsg('事件['+ rec.data.Title +']修改成功');
            							},
            							params : {
            								data : Ext.encode(rec.data)
            							}
                            ...
                            'eventdelete': {
                                fn: function(cp, rec){
                                	Ext.Ajax.request({
            							url : ctx + '/common/gycalendarevent/deleteItems.action',
            							success : function(response) {
            								var resultArray = Ext.util.JSON.decode(response.responseText);
            								App.showMsg('事件['+ rec.data.Title +']删除成功');
            							},
            							params : {
            								strChecked : rec.data.EventId
            							}
            						});
                                },
                                scope: this
                            ...
                            'eventmove': {
                                fn: function(vw, rec){
                                    ...
                                    Ext.Ajax.request({
            							...
            							success : function(response) {
            								var resultArray = Ext.util.JSON.decode(response.responseText);
            								App.showMsg('事件['+ rec.data.Title +']移动成功');
            							},
            							params : {
            								data : Ext.encode(rec.data)
            							}
...
Ext.Ajax.request({
    url : ctx + '/common/gycalendarevent/queryForManage.action',
    success : function(response) {
        var resultArray = Ext.util.JSON.decode(response.responseText);
        eventList.evts = resultArray;
