...
import com.boidea.framework.core.tplengine.AbstractTemplateEngine;
...
public class VelocityTemplateEngine extends AbstractTemplateEngine {
	...
	protected StringWriter mergeStringTemplate(DefaultTemplate pTemplate, Dto pDto) {
		...
		if(BFUtils.isEmpty(strTemplate)){
			throw new IllegalArgumentException(BFConstants.Exception_Head + "字符串模板不能为空");
		}
		...
		try {
			...
			log.error(BFConstants.Exception_Head + "字符串模板合并失败");
			e.printStackTrace();
		}
		return writer;
	}
	...
	protected StringWriter mergeFileTemplate(DefaultTemplate pTemplate, Dto pDto) {
		...
		if(BFUtils.isEmpty(filePath)){
			throw new IllegalArgumentException(BFConstants.Exception_Head + "文件模板资源路径不能为空");
		}
		StringWriter writer = new StringWriter();
		...
		try {
			...
			log.error(BFConstants.Exception_Head + "生成文件模板失败");
			e.printStackTrace();
		}
		VelocityContext context = VelocityHelper.convertDto2VelocityContext(pDto);
		try {
			...
			if(log.isDebugEnabled())log.error(BFConstants.Exception_Head + "文件模板合并失败");
			e.printStackTrace();
		} 
		return writer;
	}
