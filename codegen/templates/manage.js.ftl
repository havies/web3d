...
Ext.onReady(function() {
	...
	var qColumn1Combo = new Ext.form.ComboBox({
		name : 'qColumn1',
		hiddenName : 'qColumn1',
		store : ${zTable.className}ColumnStore,
		mode : 'local',
		triggerAction : 'all',
		valueField : 'value',
		displayField : 'text',
	...
	var expander = new Ext.grid.RowExpander({
		tpl : new Ext.Template(
			...
			mainGridHeader.${zField.fieldName} === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_${zField.fieldName}+'：</span></td><td><span>{${zField.fieldName}}</span></td></tr>':'',
			</#if>
			</#list>
			'</table>'
			),
	...
	<#list zTable.fields as zField>
	<#if zField.primaryKey?string("yes", "no") == "no">
	if(mainGridHeader.${zField.fieldName} === true)
        curColConfig.push({
                    header : mainGridHeader.header_${zField.fieldName},
                    dataIndex : '${zField.fieldName}',
                    sortable: true,
                    hidden: mainGridHeader.hide_${zField.fieldName},
                    width : 150
        });
    </#if>
    </#list>
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : './${zTable.className?lower_case}.do?reqCode=queryForManage'
						}),
				reader : new Ext.data.JsonReader({
						...
						<#list zTable.fields as zField>
							{
							 name : '${zField.fieldName}'
							}<#if zField_has_next>,</#if>
						</#list> 
							])
			});
		    ...
		    'beforerender': function (m) {
		          for (var i = 0; i < ${zTable.className}ColumnStore.getCount(); i++) {
		        	  var nItem = new Ext.menu.CheckItem({
		        		  text: ${zTable.className}ColumnStore.getAt(i).get('text'),
		        		  checked : false,
		        		  hideOnClick: false,
	...
	function selectSearchItem(checked, itemText) {
		var itemValue = '';
		for (var i = 0; i < ${zTable.className}ColumnStore.getCount(); i++) {
      	  if(itemText == ${zTable.className}ColumnStore.getAt(i).get('text')) {
      		  itemValue = ${zTable.className}ColumnStore.getAt(i).get('value');
      		  break;
      	  }
        }
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span class="commoncss">${zTable.comment}列表</span>',
				height : 500,
				// width:600,
				autoScroll : true,
				region : 'center',
				...
				tbar : [{
						...
						*/
						...
						new Ext.form.TextField({
							...
							handler : function() {
								exportExcel('./${zTable.className?lower_case}.do?reqCode=exportItem');
							}
						}],
				bbar : bbar
			});
	...
	var addFormPanel = new Ext.form.FormPanel({
				...
				items : [
						...
						,{
							id : '${zTable.pkFieldName}',
							name : '${zTable.pkFieldName}',
							xtype:"textfield",
							hidden : true
						},{
							id : 'windowmode',
	...
	var addWindow = new Ext.Window({
				...
				draggable : true,
				closeAction : 'hide',
				title : '<span class="commoncss">新增${zTable.comment}</span>',
				modal : true,
				collapsible : true,
				titleCollapse : false,
				maximizable : true,
	...
	function addInit() {
		...
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">新增${zTable.comment}</span>');
		Ext.getCmp('windowmode').setValue('add');

	}

	/**
	 * 新增${zTable.comment}数据
	 */
	function saveItem() {
		...
		addFormPanel.form.submit({
					url : './${zTable.className?lower_case}.do?reqCode=saveItem',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
						...
						store.reload();
						form.reset();
						Ext.Msg.alert('提示', '新增${zTable.comment}成功');
					},
	...
	/**
	 * 修改${zTable.comment}初始化
	 */
	function editInit() {
		var record = grid.getSelectionModel().getSelected();
		if (Ext.isEmpty(record)) {
			Ext.Msg.alert('提示', '请先选中要修改的${zTable.comment}');
			return;
		}
		addFormPanel.getForm().loadRecord(record);
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">修改${zTable.comment}</span>');
		Ext.getCmp('windowmode').setValue('edit');
		Ext.getCmp('${zTable.pkFieldName}').setValue(record.get('${zTable.pkFieldName}'));
	}

	/**
	 * 修改${zTable.comment}数据
	 */
	function updateItem() {
		if (!addFormPanel.form.isValid()) {
			return;
		...
		addFormPanel.form.submit({
                    url : './${zTable.className?lower_case}.do?reqCode=updateItem',
                    waitTitle : '提示',
                    method : 'POST',
                    waitMsg : '正在处理数据,请稍候...',
                    success : function(form, action) {
                        ...
                        store.reload();
                        form.reset();
                        Ext.Msg.alert('提示', '修改${zTable.comment}成功');
                    },
                    failure : function(form, action) {
                        top.showException(action.response.responseText);
                    }
	...
	function deleteItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.alert('提示', '请先选中要删除的${zTable.comment}!');
			return;
		}
		var strChecked = jsArray2JsString(rows, '${zTable.pkFieldName}');
		Ext.Msg.confirm('请确认', '确认删除选中的${zTable.comment}吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							url : './${zTable.className?lower_case}.do?reqCode=deleteItems',
							success : function(response) {
								var resultArray = Ext.util.JSON.decode(response.responseText);
								store.reload();
								Ext.Msg.alert('提示', '删除${zTable.comment}成功');
							},
							params : {
