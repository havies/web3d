...
Ext.onReady(function() {
	...
	var deptTree = new Ext.tree.TreePanel({
				loader : new Ext.tree.TreeLoader({
							baseAttrs : {},
							dataUrl : 'ruleRole.do?reqCode=departmentTreeInit'
						}),
				root : root,
				title : '',
				applyTo : 'deptTreeDiv',
	...
	grid.on("cellclick", function(grid, rowIndex, columnIndex, e) {
				...
				if (fieldName == 'roleid' && columnIndex == 1) {
					...
					roleGrantWindow.setTitle('<span class="commoncss">授权数据规则[角色:'+record.get('rolename')+']</span>');
				}
			});
	...
	roleGrantTabPanel.add(grid_rule);
	...
	var roleGrantWindow = new Ext.Window({
				...
				constrain : true,
				border : false,
				items : [menuTreePanel, roleGrantTabPanel],
				buttons : [{
							text : '关闭',
							iconCls : 'deleteIcon',
							handler : function() {
	...
	roleGrantWindow.on('show', function() {
	...
	function resetRuleChecked() {
		...
		Ext.Ajax.request({
			url : 'ruleRole.do?reqCode=queryRuleList4Grant',
	...
	function saveRuleItems() {
		...
		//showWaitMsg();
		Ext.Ajax.request({
			url : './ruleRole.do?reqCode=saveRuleRoleGrantDatas',
			success : function(response) {
