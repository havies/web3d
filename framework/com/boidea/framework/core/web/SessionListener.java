...
public class SessionListener implements HttpSessionListener {
	...
	public void sessionDestroyed(HttpSessionEvent event) {
		...
		//log.info("销毁了一个Session连接:" + session.getId() + " " + BFUtils.getCurrentTime());
	}
	...
	static public void addSession(HttpSession session, UserInfoVo userInfo) {
		...
		MonitorService monitorService = (MonitorService)SpringBeanLoader.getSpringBean("monitorService");
		UserInfoVo uInfo = (UserInfoVo)bfReader.queryForObject("Monitor.queryHttpSessionsByID", session.getId());
		if(BFUtils.isEmpty(uInfo)){
