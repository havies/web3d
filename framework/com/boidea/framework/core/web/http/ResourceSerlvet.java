 ...
 */
package com.boidea.framework.core.web.http;
...
import com.boidea.framework.core.datasource.EncryptablePropertyPlaceholderConfigurer;
...
public abstract class ResourceSerlvet extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("/submitLogin".equals(path)) {
            ...
            String strKey = EncryptablePropertyPlaceholderConfigurer.key;
            passwordParam = des.Encrypt(passwordParam, des.hex2byte(strKey));
        ...
        if ("".equals(path)) {
            if (contextPath == null || contextPath.equals("") || contextPath.equals("/")) {
                response.sendRedirect("/monitor/index.html");
            } else {
                response.sendRedirect("monitor/index.html");
            }
            return;
    ...
    public boolean ContainsUser(HttpServletRequest request) {
        ...
        return session.getAttribute(SESSION_USER_KEY) != null
        		|| (BFUtils.isNotEmpty(userInfo)
        				&& ("00".equals(userInfo.getRolespec())||userInfo.getRolespec().endsWith("01")));//内置角色和第一级角色身份的用户可以调用系统登录信息
    }
