...
Ext.onReady(function() {
	...
	var qColumn1Combo = new Ext.form.ComboBox({
		name : 'qColumn1',
		hiddenName : 'qColumn1',
		store : material2productColumnStore,
		mode : 'local',
		triggerAction : 'all',
		valueField : 'value',
		displayField : 'text',
	...
	var expander = new Ext.grid.RowExpander({
		tpl : new Ext.Template(
			...
			mainGridHeader.materialid === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_materialid+'：</span></td><td><span>{materialid}</span></td></tr>':'',
			mainGridHeader.productid === true?'<tr><td width="40%"><span style=color:Teal;>'+mainGridHeader.header_productid+'：</span></td><td><span>{productid}</span></td></tr>':'',
			'</table>'
			),
	...
	if(mainGridHeader.materialid === true)
        curColConfig.push({
                    header : mainGridHeader.header_materialid,
                    dataIndex : 'materialid',
                    sortable: true,
                    hidden: mainGridHeader.hide_materialid,
                    width : 150
        });
	if(mainGridHeader.productid === true)
        curColConfig.push({
                    header : mainGridHeader.header_productid,
                    dataIndex : 'productid',
                    sortable: true,
                    hidden: mainGridHeader.hide_productid,
                    width : 150
        });
    curColConfig.push({
                dataIndex : '_blank',
		    ...
		    'beforerender': function (m) {
		          for (var i = 0; i < material2productColumnStore.getCount(); i++) {
		        	  var nItem = new Ext.menu.CheckItem({
		        		  text: material2productColumnStore.getAt(i).get('text'),
		        		  checked : false,
		        		  hideOnClick: false,
	...
	function selectSearchItem(checked, itemText) {
		var itemValue = '';
		for (var i = 0; i < material2productColumnStore.getCount(); i++) {
      	  if(itemText == material2productColumnStore.getAt(i).get('text')) {
      		  itemValue = material2productColumnStore.getAt(i).get('value');
      		  break;
      	  }
        }
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span class="commoncss">material2product列表</span>',
				height : 500,
				// width:600,
				autoScroll : true,
				region : 'center',
						...
						*/
						...
						new Ext.form.TextField({
							...
							handler : function() {
								...
								exportExcelWindow(exportUrl, material2productColumnStore);
							}
						}],
				bbar : bbar
			});
	...
	var addWindow = new Ext.Window({
				...
				draggable : true,
				closeAction : 'hide',
				title : '<span class="commoncss">新增material2product</span>',
				modal : true,
				collapsible : true,
				titleCollapse : false,
				maximizable : true,
	...
	function addInit() {
		...
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">新增material2product</span>');
		Ext.getCmp('windowmode').setValue('add');

	}

	/**
	 * 新增material2product数据
	 */
	function saveItem() {
		...
		addFormPanel.form.submit({
					...
					success : function(form, action) {
						...
						store.reload();
						form.reset();
						Ext.Msg.alert('提示', '新增material2product成功');
					},
	...
	/**
	 * 修改material2product初始化
	 */
	function editInit() {
		var record = grid.getSelectionModel().getSelected();
		if (Ext.isEmpty(record)) {
			Ext.Msg.alert('提示', '请先选中要修改的material2product');
			return;
		}
		addFormPanel.getForm().loadRecord(record);
		addWindow.show();
		addWindow.setTitle('<span class="commoncss">修改material2product</span>');
		Ext.getCmp('windowmode').setValue('edit');
		Ext.getCmp('uuid').setValue(record.get('uuid'));
	}

	/**
	 * 修改material2product数据
	 */
	function updateItem() {
		if (!addFormPanel.form.isValid()) {
			return;
		...
		addFormPanel.form.submit({
                    ...
                    success : function(form, action) {
                        ...
                        store.reload();
                        form.reset();
                        Ext.Msg.alert('提示', '修改material2product成功');
                    },
                    failure : function(form, action) {
                        top.showException(action.response.responseText);
                    }
	...
	function deleteItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.alert('提示', '请先选中要删除的material2product!');
			return;
		}
		var strChecked = jsArray2JsString(rows, 'uuid');
		Ext.Msg.confirm('请确认', '确认删除选中的material2product吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
							...
							success : function(response) {
								...
								store.reload();
								Ext.Msg.alert('提示', '删除material2product成功');
							},
							params : {
